/**
 * Created by lsw on 2019/4/19 0019.
 */
import Vue from 'vue'
import Vuex from 'vuex'
import {NEXT_PAGE} from './mutation_types'
Vue.use(Vuex)
...
const actions = {
  editPageNumber({commit},index){
    commit('EDIT_PAGE_NUMBER',index)
  },
  nextPages({commit}) {
    commit('NEXT_PAGE')
  },
...
const mutations = {
  EDIT_PAGE_NUMBER(state,index){
    state.pageNumber = index
  },
  [NEXT_PAGE](state){
    state.pageNumber = Math.min(state.pageNumber + 1, state.docs.length - 1);
  },
  PREV_PAGE(state){
    state.pageNumber = Math.max(state.pageNumber - 1, 0);
  },
  REMOVE_LABEL(state, annotation){
    const index = state.annotations[state.pageNumber].indexOf(annotation);
    state.annotations[state.pageNumber].splice(index, 1);
  }
}
